...
__global__ void bfKernel(uchar* src, uchar* dst, int width, int height, int* k, int kerWidth, int kerHeight) {
	...
	//the current pixel in the image, using the dimensions of the image
	const int currentPixel = y * width + x;
	...
	const int coeff = kerWidth / 2;

	//check to make sure its NOT an edge pixel: left, right, top, bottom
	if ((x <= coeff) || (x % width < coeff) || (currentPixel / width < coeff) || (currentPixel / width > height - coeff)) {
		dst[currentPixel] = src[currentPixel];
	}
		...
		//could use loop unrolling to optimize these loops using the inverse of signs 

		int j = 0;

		//m is essentially the vertical distance the desired pixel is from the origin
		for (int m = -coeff; m <= coeff; m++) {
			//c is the horizontal distance the desired pixel is from the origin
			for (int c = -coeff; c <= coeff; c++) {
				//We find the pixels around the origin by multiplying the number of widths away the desired modifying pixel is from origin, plus the number of pixels horizontally
				//then we multiply the value at that pixel by the corresponding index in the kernel
				currentSum += src[currentPixel + (m * width + c)] * k[j];
				j++;
			}
		}

		dst[currentPixel] = (uchar)(currentSum / avgNum);
...
double GPUBoxFilter(uchar* src, uchar*& dst, int width, int height, int* k, int kerWidth, int kerHeight);
...
int main(int argc, char * argv[])
{
	...
	try {
		...
		//one side of the kernel is ___ long/high
		...
		GPUTime = GPUBoxFilter(image.data, gputmp.data, image.cols, image.rows, boxKernel, arraySize, arraySize);
		...
		cout << "In this run, the GPU ran " << (float)CPUTime / (float)GPUTime << " times faster than the CPU" << endl;
...
double GPUBoxFilter(uchar* src, uchar*& dst, int width, int height, int* k, int kerWidth, int kerHeight) {
	...
	//measuring how long it takes to render the image effect and copy the information from CPU/GPU
	HighPrecisionTime render;
	HighPrecisionTime copying;
	double renderTime = 0.0;
	double copyTime = 0.0;

	//these are variables that will be stored and used on the GPU
	uchar* GPUImageData;
	uchar* tmp;
	int* gpu_k;
	...
	//the total number of pixels in the image, using the dimensions
	int numPixels = width * height;

	//the size, in bytes, that the kernel requires
	size_t size_of_kernel = kerHeight * kerWidth * sizeof(int);
	...
	//
	//Carving out room for three variables that will be stored and used on the gpu. Their size is dependent on the size of their CPU counterparts
	//
	cudaStatus = cudaMalloc((void**)&GPUImageData, size_of_image);
	if (cudaStatus != cudaSuccess) {
		throw("cudaMalloc of image failed!");
	...
	cudaStatus = cudaMalloc((void**)&gpu_k, size_of_kernel);
	if (cudaStatus != cudaSuccess) {
		throw("cudaMalloc of image failed!");
		goto Error;
	}
	...
	//Copying the relevant information from the CPU variables to their GPU counterparts
	cudaStatus = cudaMemcpy((void*)GPUImageData, (void*)src, size_of_image, cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
		throw("cudaMemcpy of image from CPU to GPU failed!");
		goto Error;
	}

	cudaStatus = cudaMemcpy((void*)gpu_k, (void*)k, size_of_kernel, cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
		throw("cudaMemcpy of image from CPU to GPU failed!");
		goto Error;
	}

	//start timing how long it takes to render the effect
	render.TimeSinceLastCall();
	...
	// Copy output vector from GPU buffer to host memory.
	cudaStatus = cudaMemcpy((void*)dst, tmp, size_of_image, cudaMemcpyDeviceToHost);
	if (cudaStatus != cudaSuccess) {
		throw("cudaMemcpy of image data from GPU to CPU failed!");
		goto Error;
	}
